...
\newcommand{\type}[2]{\Gamma \turn{E}  #1 \asc #2}
\newcommand{\ctype}[3]{#1 \turn{E}  #2 \asc #3}

\newcommand{\typnf}[2]{\Gamma \turn{Nf}  #1 \asc #2}
...
\newcommand{\typv}[1]{\Phi \turnstyle  #1 \asc \Gamma}

\newcommand{\typwh}[2]{\Gamma \turn{Wh}  #1 \asc #2}
...
Below we provide an overview of the primary formal properties that we
wish to hold in an expressionless WHNF architecture for G{\"o}del's
System T, and motivate the properties by their roles in a
dependently typed setting. Subsequent sections will present proofs of
these primary theorems, and the additional lemmas needed for those
proofs.
